$(document).ready(function(){
	...
	MVC.M = {
		...
		boxtype: {
			grid: [],
			nav: [
				{index:'01',color:'#403b38',name:'About',img:'images/thumbs/1.jpg',tip:'美女1'},
				{index:'02',color:'#6b6360',name:'Links',img:'images/thumbs/2.jpg',tip:'美女2'},
				{index:'03',color:'#d77074',name:'Works 3D',img:'images/thumbs/3.jpg',tip:'美女3'},
				{index:'04',color:'#319ebd',name:'Works Webdesign',img:'images/thumbs/4.jpg',tip:'美女4'},
				{index:'05',color:'#92c568',name:'Works Identity',img:'images/thumbs/5.jpg',tip:'美女5'},
				{index:'06',color:'#6b6360',name:'Contact',img:'images/thumbs/6.jpg',tip:'美女6'}
			],
			subnav: []
		}
	};
	...
	MVC.C = {
		init: function(){
			$(window).bind('resize',function(){MVC.C.resize();});
			this.resize();
	...
	function GridNav(nav,x,y,tip){
		...
		function upMove(e){
			...
			function end(x,y){
					...
					x = x || Math.round(T.x/bWidthA)*bWidthA;
					y = y || Math.round(T.y/bHeightA)*bHeightA;

					if(x>wWidth-bWidthA)x-=bWidthA;
					if(y>wHeight-bHeightA)y-=bHeightA;
					...
					if(flag && T.isDraged){//如果有查找周围
						var _arr = [
							{x:(x+bWidthA>wWidth-bWidthA?x-2*bWidthA:x+bWidthA),y:y},
							{x:x,y:(y+bHeightA>wHeight-bHeightA?y-2*bHeightA:y+bHeightA)},
							{x:(x-bWidthA<0?x+2*bWidthA:x-bWidthA),y:y},
							{x:x,y:(y-bHeightA<0?y+2*bHeightA:y-bHeightA)}
						];
						for (var i = 0; i < _arr.length; i++) {
							flag = false;
							for (var j = 0; j < vNavs.length; j++) {
								if (vNavs[j].isDraged) {continue;}
								if(_arr[i].x == vNavs[j].x && _arr[i].y == vNavs[j].y){flag = true;break;}
							}
							...
							if (!flag) {
								...
								y = _arr[i].y;
								console.log('x: '+x+' y: '+y)
								//arguments.callee(_arr[i].x,_arr[i].y);
								break;
							}
						}
						//return;
