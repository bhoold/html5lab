...
(function($, undefined) {
	...
	$.jPlayer.prototype = {
		...
		_init: function() {
			...
			if(this.flash.used) {
				...
				if($.browser.msie && Number($.browser.version) <= 8) {
					...
					obj_param[5] = '<param name="wmode" value="transparent" />';
		...
		videoPlay: function(e) { // Handles clicks on the play button over the video poster
			// ravorona: show/hide controller on rollover.
			$(this.element).parent().find('.jp-controller').animate({bottom: 0}, {queue: false, duration: 500, easing: 'linear'});
			$(this.element).parent().hover(
				function()
				{
					$(this).find('.jp-controller').animate({bottom: 0}, {queue: false, duration: 500, easing: 'linear'});;
				},
				function()
				{
					$(this).find('.jp-controller').animate({bottom: -60}, {queue: false, duration: 500, easing: 'linear'});;
				}
			);
		...
		_html_checkWaitForPlay: function() {
			if(this.status.waitForPlay) {
				...
				if(this.status.video) {
					...
					this.internal.video.jq.css({'width': '100%', 'height': '100%'});
				}
			}
		},
		...
		_flash_checkWaitForPlay: function() {
			if(this.status.waitForPlay) {
				...
				if(this.status.video) {
					...
					this.internal.flash.jq.css({'width': '100%', 'height': '100%'});
				}
			}
		},
