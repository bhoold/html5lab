      ...
      var playerParameters = ['autohide', 'autoplay', 'border', 'cc_load_policy', 'color', 'controls', 'disablekb', 'enablejsapi', 'egm', 'fs', 'hd', 'iv_load_policy', 'loop', 'origin', 'playerapiid', 'playlist', 'rel', 'showinfo', 'showsearch', 'start'];
      var unsupported = {
                          'AS2': {
                            'controls': 1,
                            'origin': 1,
      ...
      function constructUrl(baseUrl) {
        vid = getParamValue('v') ? getParamValue('v') : 'u1zgFlCw8Aw';
       ...
       */
      function showPlayerParameters(selectId) {
        ...
        for (var count in playerParameters) {
          ...
          if (parameterName && parameterDefinition && parameterLink) {
            if (playerVersion && unsupported[playerVersion] && unsupported[playerVersion][playerParameters[count]]) {
          ...
          if (updateParams) {
            ...
            for (var i = 0, input; input = inputs[i]; i++) {
              ...
              if (paramName == 'width') {
                ...
                paramValue = '1';
              } else if (paramName == 'start' || paramName == 'playerapiid' ||
                  paramName == 'playlist') {
                if (!paramValue) {
                  continue;
                }
